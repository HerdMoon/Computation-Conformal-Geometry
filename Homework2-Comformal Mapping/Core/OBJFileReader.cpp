...
void OBJFileReader::writeToSolid(Solid *mesh, std::ostream &out)
{
	int vObjID = 1;
	...
	out << "# " << (unsigned int)mesh->numVertices() << " vertices" << std::endl;
	...
	out << "# " << (unsigned int)mesh->numVertices() << " texture coordinates" << std::endl;
	...
	for (; !fiter2.end(); ++fiter2)
	{
		...
		{
			Vertex *v = *viter;
			out << vidToObjID[v->id()] << "/" << vidToObjID[v->id()] << " ";
		}
		out << std::endl;
	}
}